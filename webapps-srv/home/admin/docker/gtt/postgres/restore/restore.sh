#!/usr/bin/env bash
# Encoding : UTF-8
# Script to restore GTT database custom dump into Postgresql container.
...
function setConstants() {
    readonly grs_dump_file="${setting_dump_file-${POSTGRES_RESTORE_FILE-'gtt.dump'}}"
...
function restoreDbDump() {
    ...
    local db_to_restore=""
    if [[ "${grs_dump_file}" =~ .*"gtt".* ]]; then
        db_to_restore="gtt"
    else
        exitScript "Unknown database name in ${grs_dump_file} ! Known : gtt."
    fi

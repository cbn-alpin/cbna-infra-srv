...
# Année : 2026
# Script de sauvegarde de l’instance GTT
...
repertoire_sauvegarde="/home/admin/docker/gtt/backups"
date_du_jour=$(date +%F)
...
# conteneurs utilisés
conteneur_bdd="gtt-postgres"
conteneur_data="gtt-api"

# image contenant pg_dump
image_dump="postgres:18.1"
port_bdd=5432
...
echo "Copie des fichiers statiques et médias de GTT..."
docker cp "${conteneur_data}:/home/app/web/migrations/data" "${dossier_temporaire}/migrations-data"
echo ""

echo "Archivage des fichiers statiques et médias..."
if ! tar czf "${dossier_cible}/${nom_sauvegarde}.media-static.tar.gz" -C "${dossier_temporaire}" migrations-data; then
    echo "Erreur : Archivage échoué."
    exit 1
fi
echo "Archive créée : ${nom_sauvegarde}.migrations-data.tar.gz"
echo ""
